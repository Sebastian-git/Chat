/*
 * Deals with in coming connections to server class
 */
		...
		try {
			...
			synchronized (this) {
				for (Connection i : server.connections) {
					if (i != null) {
						if (i != this) {
							i.out.println("NEWUSER");
							i.out.println(username);
						}
						out.println("NEWUSER");
			...
			synchronized (this) {
				for (Connection i : server.connections) {
					if (i != null) {
						i.out.println(username + " has left the server. There are now " + (totalConnected - 1) + "/"
								+ Server.MAX_CONNECTIONS + " online.");
					}
				}
			}
			...
			synchronized (this) {
				for (Connection i : server.connections) {
					if (i != null && i != this) {
						i.out.println("DELUSER");
						i.out.println(this.username);
					}
				}
			}
